...
export async function createTask(newTask: { name: string; content: string; startDate: string; endDate: string; }) {
  try {
    const response: AxiosResponse = await apiClient.post("/post");
    return response.data;
  } catch (error) {
    const err = error as AxiosError;
    throw new Error(`Error fetching message: ${err.response?.data}`);
